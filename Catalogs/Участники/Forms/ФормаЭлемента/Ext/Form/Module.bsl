...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		//ТекущийРейтинг = Серверные.ПолучитьТекущийРейтингУчастника(Объект.Ссылка,ТекущаяДата());
		ТекущийРазряд = Серверные.ПолучитьРазрядЗваниеУчастника(Объект.Ссылка,ТекущаяДата());
...
Процедура ПосчитатьКоличествоЛет()
	...
	Если Объект.ДатаРождения <> Дата('00010101') Тогда
		...
		//Выборка.Следующий();
		//
		//КолДнейСоДняРождения = Выборка.КолДней;
		//
		//Элементы.КолЛет.Заголовок = ""+(ТекГод-ГодРождения)+" лет";
	КонецЕсли;
...
Процедура НаименованиеПриИзменении(Элемент)
	...
	Фамилия 	 = Общий.ВыделитьСлово(ФИО);
	Имя     		 = Общий.ВыделитьСлово(ФИО);
	Отчество 	 = Общий.ВыделитьСлово(ФИО);
...
Процедура СсылкаНаКартинкуНажатиеЗавершение1(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если (ВыбранныеФайлы <> Неопределено) Тогда
		НачатьПомещениеФайла(Новый ОписаниеОповещения("СсылкаНаКартинкуНажатиеЗавершение", ЭтотОбъект, Новый Структура("АдресКартинки", АдресКартинки)), АдресКартинки, Диалог.ПолноеИмяФайла,Ложь, Новый УникальныйИдентификатор);
	КонецЕсли;
...
Процедура СсылкаНаКартинкуНажатиеЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	СсылкаНаКартинку = АдресКартинки;
