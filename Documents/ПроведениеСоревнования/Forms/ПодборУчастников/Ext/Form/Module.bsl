...
Процедура Декорация4Нажатие(Элемент)
	...
	ОткрытьФорму("Справочник.Участники.ФормаОбъекта",,ЭтотОбъект,,ВариантОткрытияОкна.ОтдельноеОкно,,Описание);
КонецПроцедуры

&НаКлиенте
Процедура ПослеЗакрытияФормы(Результат,ДопПараметры)
...
Процедура СтрокаПоискаОчистка(Элемент, СтандартнаяОбработка)
	ОтборСписка = СписокУчастниковДляПодбора.Отбор.Элементы;
	Для Каждого ТекОтбор Из ОтборСписка Цикл
		Если ТекОтбор.ЛевоеЗначение =  Новый ПолеКомпоновкиДанных("Участник.Наименование") Тогда
			ОтборСписка.Удалить(ТекОтбор);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура СтрокаПоискаАвтоПодбор(Элемент, Текст, ДанныеВыбора, Ожидание, СтандартнаяОбработка)
	...
	Если Текст <> "" Тогда
		//поиск
		УсловиеОтбора = ОтборСписка.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		УсловиеОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Участник.Наименование");
