...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() и Не ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
		Объект.Ответственный 		= ПараметрыСеанса.ТекПользователь;
	...
	Иначе
		Если ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда //это копирование
			...
			Для каждого ТекСтр Из Объект.СписокУчастников Цикл
				ТекСтр.ТекущийРейтинг = Серверные.ПолучитьТекущийРейтингУчастника(ТекСтр.Участник,ТекущаяДата(),Объект.ВидРейтинга);
			КонецЦикла;
...
Процедура ОбновитьКнопкуПроведения()
	Если Объект.Проведен Тогда //значит рассчитан
		Элементы.ФормаКомандаРасчитатьРейтинг.Заголовок = "Рейтинг рассчитан";
		Элементы.ФормаКомандаРасчитатьРейтинг.ЦветФона	= Новый Цвет(255,225,0);
	Иначе
		Элементы.ФормаКомандаРасчитатьРейтинг.Заголовок = "Рассчитать рейтинг";
		Элементы.ФормаКомандаРасчитатьРейтинг.ЦветФона	= Новый Цвет(255,249,204);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПодборУчастников(Команда)
	...
	ОткрытьФорму("Документ.ПроведениеСоревнования.Форма.ПодборУчастников",ПараметрыФормы,ЭтотОбъект,Новый УникальныйИдентификатор,,,,Серверные.ПолучитьРежимОткрытияФормыПодбора());
КонецПроцедуры
	...
	ИначеЕсли ИсточникВыбора.ИмяФормы = "Документ.ПроведениеСоревнования.Форма.УниверсальнаяФормаЖеребьевки" Тогда
		...
		Если ВыбранноеЗначение.ВидСоревнований = ПредопределенноеЗначение("Перечисление.ВидыСоревнований.Личные") Тогда
			...
			Если ВыбранноеЗначение.ЧисловоеЗначениеТура = 1 Тогда
				...
				ОткрытьФорму("Документ.ПроведениеСоревнования.Форма.ФормаПроведенияСоревнования",ПараметрыФормы,ЭтотОбъект,,ВариантОткрытияОкна.ОтдельноеОкно);
			Иначе
				Для Каждого ТекСтрока Из ВыбранноеЗначение.ВыбранныеУчастники Цикл
					НовСтрока = Объект.УчастникиПоЖеребьевке.Добавить();
					НовСтрока.РежимТура   	 = ВыбранноеЗначение.РежимТура;
				...
				КонецЕсли; 
				ОткрытьФорму("Документ.ПроведениеСоревнования.Форма.ФормаМинус2ИОлимпийка",ПараметрыФормы,ЭтотОбъект,,ВариантОткрытияОкна.ОтдельноеОкно);
			КонецЕсли;
		ИначеЕсли ВыбранноеЗначение.ВидСоревнований = ПредопределенноеЗначение("Перечисление.ВидыСоревнований.Командные") Тогда
			...
			Если ВыбранноеЗначение.ЧисловоеЗначениеТура = 1 Тогда
				...
				ОткрытьФорму("Документ.ПроведениеСоревнования.Форма.ФормаПроведенияСоревнованияКоманды",ПараметрыФормы,ЭтотОбъект,,ВариантОткрытияОкна.ОтдельноеОкно);
			Иначе
				Для Каждого ТекСтрока Из ВыбранноеЗначение.ВыбранныеУчастники Цикл
					НовСтрока = Объект.КомандыПоЖеребьевке.Добавить();
					НовСтрока.РежимТура   	 = ВыбранноеЗначение.РежимТура;
				...
				ПараметрыФормы.Вставить("ВидСоревнований",ВыбранноеЗначение.ВидСоревнований);
				ПараметрыФормы.Вставить("ЖеребьевкаКоманд",ВыбранноеЗначение.ЖеребьевкаКоманд);
				ОткрытьФорму("Документ.ПроведениеСоревнования.Форма.ФормаМинус2ИОлимпийкаКоманды",ПараметрыФормы,ЭтотОбъект,,ВариантОткрытияОкна.ОтдельноеОкно);
			КонецЕсли;
		КонецЕсли; 
		...
		Если втТекКоманда.Пометка Тогда
			...
			Для Каждого втТекУчастник Из СтрокиСУчастниками Цикл
				Если втТекУчастник.Пометка Тогда //добавляем в таблицу команд
					...
					НовСтрока.ТекущийРейтинг		= Серверные.ПолучитьТекущийРейтингУчастника(НовСтрока.Участник,ТекущаяДата(),Объект.ВидРейтинга);
				КонецЕсли; 
			КонецЦикла; 
		КонецЕсли; 	
	КонецЦикла;
...
Процедура ЗаполнитьТаблицуДанныхИзТабличногоДокумента(ДанныеДляЭкспорта,ДанныеЗагрузки)
	...
	Пока ЗагрузкаИдет Цикл
		...
		КонецЕсли;
		ИтоговыйПарДаты = Общий.ПриведениеИзСтрокиВДату(парДатаРождения);
...
Процедура ДобавитьВСписокЭкспортнуюТаблицу(ДанныеЭкспорта,ТаблицаЭкспорта)
	...
	Для Каждого ТекЗначение Из ТаблицаЭкспорта Цикл
		...
		Запрос.УстановитьПараметр("Дата",ТекЗначение.ДатаРождения);
		...
		Если Результат.Количество() = 0 Тогда
			...
			КонецЕсли; 
		Иначе
			Участник = Результат[0].Ссылка;
		КонецЕсли;
		Если ДанныеЭкспорта.ЗагружатьИЗФайлаРейтинг = 1 Тогда
			мРейтинг = Серверные.ПолучитьТекущийРейтингУчастника(Участник,ТекущаяДата(),Объект.ВидРейтинга);
		Иначе
...
&НаКлиенте
Процедура УдалитьДанныеПоЭтапуНаСервере(НомерТура)
	...
	Если мНомерУдаляемойСтрокиИзХодаСоревнований <= Объект.ХодСоревнования.Количество() - 1 Тогда
		Для Каждого ТекСтрока Из Объект.УчастникиПоЖеребьевке Цикл
			Если ТекСтрока.НомерТура >= мНомерУдаляемойСтрокиИзХодаСоревнований Тогда
				...
				//ТекСтрока.РежимТура = Объект.ХодСоревнования.Получить(ТекСтрока.НомерТура).РежимТура;
			КонецЕсли;
		КонецЦикла;
...
Процедура ХодСоревнованияПередУдалением(Элемент, Отказ)
	...
	Если МассивВЖеребьевке.Количество() > 0 Или МассивВОбщейТаблице.Количество() > 0 Тогда
		Отказ = Истина;
		Ответ = Неопределено;
		ПоказатьВопрос(Новый ОписаниеОповещения("ХодСоревнованияПередУдалениемЗавершение", ЭтотОбъект, Новый Структура("ТекСтрока", ТекСтрока)), "Все данные по этапу будут удалены. Удалить?",РежимДиалогаВопрос.ДаНет);
	Иначе
		ЭтотОбъект.Модифицированность = Истина;
	КонецЕсли; 
...
Процедура ХодСоревнованияПередУдалениемЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		УдалитьДанныеПоЭтапуНаСервере(ТекСтрока.НомерСтроки);
...
&НаКлиенте
Процедура ХодСоревнованияКолПартийНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	//Список.Добавить(5);
	//Список.Добавить(7);
	//ВыбранноеЗначение = ЭтотОбъект.ВыбратьИзСписка(Список,ЭтотОбъект);
	//Если ВыбранноеЗначение <> Неопределено Тогда
	//	Элементы.ХодСоревнования.ТекущиеДанные.КолПартий = ВыбранноеЗначение.Значение;
	//КонецЕсли;
...
Процедура ХодСоревнованияПриАктивизацииЯчейки(Элемент)
	Если Элементы.ХодСоревнования.ТекущиеДанные <> Неопределено Тогда
		...
		КонецЕсли;
		//Если Элементы.ХодСоревнованияПосмотретьЭтап.Заголовок <> ТекстЗаголовка Тогда
		//	Элементы.ХодСоревнованияПосмотретьЭтап.Заголовок = ТекстЗаголовка;
		//КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура РезультатЖеребьевки(Команда)
	...
	ПараметрыФормы.Вставить("СеткаНа",ДанныеСтроки.СеткаНа);
	глКолПартий = ДанныеСтроки.КолПартий;
	ОткрытьФорму("Документ.ПроведениеСоревнования.Форма.УниверсальнаяФормаЖеребьевки",ПараметрыФормы,ЭтотОбъект,Новый УникальныйИдентификатор,ВариантОткрытияОкна.ОтдельноеОкно);
	
КонецПроцедуры

//БЛОК ПЕЧАТИ
&НаКлиенте
Процедура ПечатьНастройки(Команда)
	ДанныеДляФормы = Новый Структура("АдресДокОбъект",ПоместитьВоВременноеХранилище(Объект));
	ОткрытьФорму("Документ.ПроведениеСоревнования.Форма.НастройкиПечати",ДанныеДляФормы,ЭтотОбъект,Новый УникальныйИдентификатор,ВариантОткрытияОкна.ОтдельноеОкно);
КонецПроцедуры
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		//проверяем вид рейтинга
		//Если Объект.ВидРейтинга = ПредопределенноеЗначение("Справочник.ВидыРейтинга.ПустаяСсылка") Тогда
		//	Ответ = Вопрос("Не заполнен обсчитываемый рейтинг. Продолжить?",РежимДиалогаВопрос.ДаНет);
...
Процедура РассчитатьСреднийРейтинг()
	...
	ДанныеДляСреднегоРейтинга = Документы.ПроведениеСоревнования.РассчитатьСреднийРейтингВМодулеМенеджера(Объект.СписокУчастников.Выгрузить());
...
Процедура ПерерасчитатьПриростыИИтоговыйРейтинг(ИДСтрокиПерерасчета = Неопределено)
	...
	Если ТекСтрока.КолПартий1 > ТекСтрока.КолПартий2 Тогда
		Выигравший = ТекСтрока.Участник1;
		РейтВыиграв = ТекСтрока.ТекущийРейтинг1;
		ПриростВыигравшего = ТекСтрока.Прирост1;
		Проигравший = ТекСтрока.Участник2;
		РейтПроиграв = ТекСтрока.ТекущийРейтинг2;
		ПриростПроигравшего = ТекСтрока.Прирост2;
	Иначе
		Выигравший = ТекСтрока.Участник2;
		РейтВыиграв = ТекСтрока.ТекущийРейтинг2;
		ПриростВыигравшего = ТекСтрока.Прирост2;
		Проигравший = ТекСтрока.Участник1;
		РейтПроиграв = ТекСтрока.ТекущийРейтинг1;
		ПриростПроигравшего = ТекСтрока.Прирост1;
	КонецЕсли; 
	
	мОбъект = РеквизитФормыВЗначение("Объект");
	
	Если ТекСтрока.КолПартий1 > ТекСтрока.КолПартий2 Тогда
		ТекСтрока.Прирост1 = Объект.Коэфф * мОбъект.РассчитатьПрирост(РейтВыиграв,РейтПроиграв);
		ТекСтрока.Прирост2 = Объект.Коэфф * мОбъект.РассчитатьПрирост(РейтВыиграв,РейтПроиграв,Ложь);
	Иначе
		ТекСтрока.Прирост2 = Объект.Коэфф * мОбъект.РассчитатьПрирост(РейтВыиграв,РейтПроиграв);
		ТекСтрока.Прирост1 = Объект.Коэфф * мОбъект.РассчитатьПрирост(РейтВыиграв,РейтПроиграв,Ложь);
	КонецЕсли;
	ОбновитьНовыйРейтингВСпискеУчастников();
...
Процедура КомандаЭкспортИзExcel(Команда)
	ОткрытьФорму("Документ.ПроведениеСоревнования.Форма.НастройкиЭкспортаСписка",,ЭтотОбъект,,,,Новый ОписаниеОповещения("ПослеВыбораНастроекЗагрузки",ЭтотОбъект));
КонецПроцедуры
...
Процедура КомандаЗаполнитьБонусы(Команда)
	...
	Если Объект.ОбщийРежимСоревнования = 0 Тогда
		Если Объект.Бонусы.Количество() > 0 Тогда
			Ответ = Неопределено;
			ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗаполнитьБонусыЗавершение2", ЭтотОбъект), "Уже есть заполненные бонусы, они будут очищены.Продолжить?",РежимДиалогаВопрос.ДаНет);
		КонецЕсли;
...
Процедура КомандаЗаполнитьБонусыФрагмент1(Знач ВыбранныйЭтап)
	...
	Перем ВсегоГруппТура, ВыбраннаяГруппа, ГруппаТураДляЗаполнения, м, РежимТура, СписокГрупп, СтрокаТаблицы;
	...
	Если ВыбранныйЭтап <> Неопределено Тогда
		...
		Если РежимТура = ПредопределенноеЗначение("Перечисление.РежимыТуровСоревнования.Групповой") Тогда
			...
			Иначе
				...
				СписокГрупп.ПоказатьВыборЭлемента(Новый ОписаниеОповещения("КомандаЗаполнитьБонусыЗавершение", ЭтотОбъект, Новый Структура("ВыбранныйЭтап", ВыбранныйЭтап)), "Выберите группу для бонусов");
				Возврат; 
			КонецЕсли; 
		Иначе
...
Функция ПолучитьКоличествоГруппТура(НомТура)
	...
	Возврат Документы.ПроведениеСоревнования.ОпределитьКоличествоГрупп(Объект.УчастникиПоЖеребьевке.Выгрузить(),НомТура,Перечисления.РежимыТуровСоревнования.Групповой);
...
Процедура ЗаполнитьУчастниковПоМестамИБонусам(НомЭтапа = 1,НомГруппы = 0)
	...
	Если РежимТура = Перечисления.РежимыТуровСоревнования.Групповой Тогда
		//Н = 1;
		мОбщаяТаблицаПоТуру = Объект.ОбщаяТаблицаДанных.Выгрузить(Новый Структура("НомерТура,НомерГруппы",НомерТура,НомГруппы));
		...
		КоличествоУчастников = Объект.УчастникиПоЖеребьевке.НайтиСтроки(Новый Структура("НомерГруппы,НомерТура,РежимТура",НомГруппы,НомерТура,Перечисления.РежимыТуровСоревнования.Групповой));
		Ном = 1;
	...
	|ИЗ
	|	РегистрСведений.БонусныеОчки.СрезПоследних КАК БонусныеОчкиСрезПоследних
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура ТаблицаБыстрогоРасчетаУчастник1АвтоПодбор(Элемент, Текст, ДанныеВыбора, Ожидание, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ДанныеВыбора = ВозвратДанных(Текст);
КонецПроцедуры

&НаСервере
Функция ВозвратДанных(мТекст)
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.Текст = "ВЫБРАТЬ * ПОМЕСТИТЬ втТЗ ИЗ &Внеш КАК Внеш";
	Если Объект.ОбщийРежимСоревнования = 0 Или Объект.ОбщийРежимСоревнования = 1 Тогда
		Запрос.УстановитьПараметр("Внеш",Объект.СписокУчастников.Выгрузить());
	ИначеЕсли Объект.ОбщийРежимСоревнования = 2 Тогда
		Запрос.УстановитьПараметр("Внеш",Объект.ТаблицаКоманд.Выгрузить());
	КонецЕсли; 
	Запрос.Выполнить();
	...
	|ГДЕ
	|	ПроведениеСоревнованияСписокУчастников.Участник.Наименование ПОДОБНО ""%"" + &Наименование + ""%""";
	Запрос.УстановитьПараметр("Наименование",мТекст);
	Запрос.Текст = Текст;
	Результат = Запрос.Выполнить().Выгрузить();
	
	Список = Новый СписокЗначений;
	Для каждого ТекСтр Из Результат Цикл
		Если ЗначениеЗаполнено(ТекСтр.Участник.ДатаРождения) Тогда
			Список.Добавить(ТекСтр.Участник,""+ТекСтр.Участник+" ("+Формат(ТекСтр.Участник.ДатаРождения,"ДФ=dd.MM.yy")+")",,БиблиотекаКартинок.Реквизит);
		Иначе
			Список.Добавить(ТекСтр.Участник,""+ТекСтр.Участник,,БиблиотекаКартинок.Реквизит);
		КонецЕсли; 
	КонецЦикла; 
...
Процедура КомандаОткрытьТурКомандный(Команда)
	...
	Если Элементы.ХодСоревнованияКоманды.ТекущиеДанные <> Неопределено Тогда
		Если Элементы.ХодСоревнованияКоманды.ТекущиеДанные.РежимТура <> ПредопределенноеЗначение("Перечисление.РежимыТуровСоревнования.ПустаяСсылка") Тогда
			...
			Если ИскомыеСтроки.Количество() > 0 Тогда
				Если ДанныеСтроки.РежимТура = ПредопределенноеЗначение("Перечисление.РежимыТуровСоревнования.Групповой") Тогда 
					ОткрытьФорму("Документ.ПроведениеСоревнования.Форма.ФормаПроведенияСоревнованияКоманды",ПараметрыФормы,ЭтотОбъект,Новый УникальныйИдентификатор,ВариантОткрытияОкна.ОтдельноеОкно);
				Иначе
					ОткрытьФорму("Документ.ПроведениеСоревнования.Форма.ФормаМинус2ИОлимпийкаКоманды",ПараметрыФормы,ЭтотОбъект,Новый УникальныйИдентификатор,ВариантОткрытияОкна.ОтдельноеОкно);
				КонецЕсли;
			Иначе
				ОткрытьФорму("Документ.ПроведениеСоревнования.Форма.УниверсальнаяФормаЖеребьевки",ПараметрыФормы,ЭтотОбъект,Новый УникальныйИдентификатор,ВариантОткрытияОкна.ОтдельноеОкно);
			КонецЕсли;
...
Процедура ОчиститьБыстрыйРасчет(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ОчиститьБыстрыйРасчетЗавершение", ЭтотОбъект), "Будут очищены все игры в быстром режиме. Очистить?",РежимДиалогаВопрос.ДаНет); 
КонецПроцедуры
...
Процедура КомандаОбновитьРейтинг(Команда)
	...
	ПоказатьВводДаты(Новый ОписаниеОповещения("КомандаОбновитьРейтингЗавершение", ЭтотОбъект, Новый Структура("мТекДата", мТекДата)), мТекДата,"Рейтинг будет обновлен на",ЧастиДаты.Дата);
КонецПроцедуры
...
Процедура ОбновитьРейтингВСпискеУчастников(мДатаОбновления = Неопределено)
	Для каждого ТекСтр Из Объект.СписокУчастников Цикл
		ТекСтр.ТекущийРейтинг = Серверные.ПолучитьТекущийРейтингУчастника(ТекСтр.Участник,?(мДатаОбновления = Неопределено,ТекущаяДата(),мДатаОбновления),Объект.ВидРейтинга);
	КонецЦикла; 	
	РассчитатьСреднийРейтинг();
КонецПроцедуры
...
Процедура КомандаПеренестиЛичныеИгрыВБыстрыйРасчет(Команда)
	...
	Если СписокТуров.Количество() > 0 Тогда
		СписокТуров.ПоказатьОтметкуЭлементов(Новый ОписаниеОповещения("КомандаПеренестиЛичныеИгрыВБыстрыйРасчетЗавершение1", ЭтотОбъект, Новый Структура("СписокТуров", СписокТуров)), "Выберите этапы для переноса."); 
	КонецЕсли;
...
Процедура КомандаПеренестиЛичныеИгрыВБыстрыйРасчетЗавершение1(Список, ДополнительныеПараметры) Экспорт
	...
	Если Объект.ТаблицаБыстрогоРасчета.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаПеренестиЛичныеИгрыВБыстрыйРасчетЗавершение", ЭтотОбъект, Новый Структура("СписокТуров", СписокТуров)), "В быстром расчете уже есть данные, что будем делать?",СписокКнопок,40);
		Возврат; 
	КонецЕсли; 
...
Процедура ПеренестиЛичныеВстречи(парСписокТуров)
	...
	|			ИЛИ ПроведениеСоревнованияОбщаяТаблицаДанных.ТехническоеП = ЛОЖЬ)
...
Процедура ХодСоревнованияКомандыКолПартийНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ЭтотОбъект.ПоказатьВыборИзСписка(Новый ОписаниеОповещения("ХодСоревнованияКомандыКолПартийНачалоВыбораЗавершение", ЭтотОбъект), Список,ЭтотОбъект);
КонецПроцедуры
...
Процедура ХодСоревнованияКомандыПриАктивизацииЯчейки(Элемент)
	Если Элементы.ХодСоревнованияКоманды.ТекущиеДанные <> Неопределено Тогда
		...
		МассивСтрок = Объект.КомандыПоЖеребьевке.НайтиСтроки(Новый Структура("НомерТура,РежимТура",ТекСтрока.НомерСтроки,ТекСтрока.РежимТура));
		Если МассивСтрок.Количество() > 0 Тогда
...
Процедура ХодСоревнованияКомандыПередУдалением(Элемент, Отказ)
	...
	Если МассивВЖеребьевке.Количество() > 0 Или МассивВОбщейТаблице.Количество() > 0 Тогда
		Ответ = Неопределено;
		ПоказатьВопрос(Новый ОписаниеОповещения("ХодСоревнованияКомандыПередУдалениемЗавершение", ЭтотОбъект, Новый Структура("ТекСтрока", ТекСтрока)), "Все данные по этапу будут удалены. Удалить?",РежимДиалогаВопрос.ДаНет);
	Иначе
		ЭтотОбъект.Модифицированность = Истина;
	КонецЕсли;
...
Процедура ВыбратьКомандыИзИстории(Команда)
	...
	ОткрытьФорму("Документ.ПроведениеСоревнования.Форма.ИсторияСоставовКоманд",ПараметрыДляФормы,ЭтотОбъект,,ВариантОткрытияОкна.ОтдельноеОкно);
КонецПроцедуры
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Не ЗавершениеРаботы Тогда
		Серверные.УдалитьВсеТекущиеВстречи(ЭтотОбъект.УникальныйИдентификатор);
	КонецЕсли; 
