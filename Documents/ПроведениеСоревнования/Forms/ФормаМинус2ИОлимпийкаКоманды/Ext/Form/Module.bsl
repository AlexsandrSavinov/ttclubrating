...
Процедура ЗаполнениеТаблицУчастниковИИгр(Область)
	...
	СводнаяТаблица1 = Запрос.Выполнить().Выгрузить();
	...
	Если ТаблицаМестИгр.Количество() > 0 Тогда
		Если СсылкаНаДок.Проведен Тогда //значит рейтинг рассчитан уже
			 ...
			 Если ИскомыйНомер.Количество() > 0 Тогда
				 //ищем рейтинг участника по этом соревнованию
				 Запрос = Новый Запрос;
				 Запрос.Текст = "ВЫБРАТЬ
...
Процедура УдалитьПартию(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("УдалитьПартиюЗавершение", ЭтотОбъект, Новый Структура("ДанныеПоСтроке, ТекСтрока", ДанныеПоСтроке, ТекСтрока)), "Вы уверены, что хотите удалить результат игры между
...
Процедура ТаблицаПартийТехническоеПриИзменении(Элемент)
	...
	Если ЗначениеСтроки Тогда
		...
		ТекДанные.Итог = "0";
		ИтогСтроки = ОпределитьКолПартий();
		ВтораяСтрока.Итог = ИтогСтроки;
	КонецЕсли;
	//КомандаЗаписатьПартию();
...
Процедура ТаблицаПартийУчастник1НачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ЭтотОбъект.ПоказатьВыборИзСписка(Новый ОписаниеОповещения("ТаблицаПартийУчастник1НачалоВыбораЗавершение", ЭтотОбъект, Новый Структура("ТекСтрока", ТекСтрока)), мПеремСписок,Элемент);
...
Процедура ТаблицаПартийУчастник2НачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ЭтотОбъект.ПоказатьВыборИзСписка(Новый ОписаниеОповещения("ТаблицаПартийУчастник2НачалоВыбораЗавершение", ЭтотОбъект, Новый Структура("ТекСтрока", ТекСтрока)), мПеремСписок,Элемент);
...
Процедура ПоменятьРасстановку(Команда)
	...
	Если МассивСтрок.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ПоменятьРасстановкуЗавершение", ЭтотОбъект, Новый Структура("МассивСтрок", МассивСтрок)), "У этих команд уже есть данные, по жеребьевке участников.",СписокОтветов); 
	Иначе
...
Процедура КомандаСкопироватьПредыдущуюРасстановку(Команда)
	...
	Если ЗначениеЗаполнено(НаименованиеКоманда1) И ЗначениеЗаполнено(НаименованиеКоманда2) Тогда
		ТекстВопроса = "Заполнить по команде(ам)?"
	Иначе
		ТекстВопроса = "Заполнить по команде?"
	КонецЕсли;
	ОтветСписка = Неопределено;

	СписокОтветов.ПоказатьВыборЭлемента(Новый ОписаниеОповещения("КомандаСкопироватьПредыдущуюРасстановкуЗавершение", ЭтотОбъект), "Выберите команду?"); 
...
Процедура КомандаСкопироватьПредыдущуюРасстановкуЗавершение(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	//Ответ = Вопрос(ТекстВопроса,СписокОтветов,,,"Выберите команду");
