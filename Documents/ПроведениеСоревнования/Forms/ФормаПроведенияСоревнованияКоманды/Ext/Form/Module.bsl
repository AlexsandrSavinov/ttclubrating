...
&НаКлиенте
Функция ПолучитьДанныеПоНомеруГруппы(НомГруппы = 1)
	
	Данные = Новый Структура;
	Данные.Вставить("НомГруппы",НомГруппы);
	
	втТаблицаГруппы = ЭтотОбъект["РеквизитТаблицы_"+НомГруппы]; 
	втЭлементТаблицаГрупп = Элементы["Таблица_"+НомГруппы];
	втТекСтрокаГрупп = втЭлементТаблицаГрупп.ТекущиеДанные;
	
	Данные.Вставить("ТаблицаГруппы",втТаблицаГруппы);
	Данные.Вставить("ЭлементТаблицаГрупп",втЭлементТаблицаГрупп);
	Данные.Вставить("ТекСтрокаГрупп",втТекСтрокаГрупп);
	//-------------------------------------------------------------
	//втТаблицаПартий = ЭтотОбъект["РеквизитТаблицаПартий_"+НомГруппы];
	//втЭлементТаблицаПартий = Элементы["ТаблицаИгр_"+НомГруппы];
	//втТекСтрокаПартии = втЭлементТаблицаПартий.ТекущиеДанные;
	//
	Данные.Вставить("ТаблицаПартий",ТаблицаПартий);
	Данные.Вставить("ЭлементТаблицаПартий",Элементы.ТаблицаПартий);
	Данные.Вставить("ТекСтрокаПартии",Элементы.ТаблицаПартий.ТекущиеДанные);
	//
	//Данные.Вставить("ОбщийСчет1",ЭтотОбъект["РеквизитОбщийКомандныйСчет1_"+НомГруппы]);
	//Данные.Вставить("ОбщийСчет2",ЭтотОбъект["РеквизитОбщийКомандныйСчет2_"+НомГруппы]);
	Данные.Вставить("ОбщийСчет1",СуммаИтогов1);
	Данные.Вставить("ОбщийСчет2",СуммаИтогов2);
	//
	//Данные.Вставить("Команда1",ЭтотОбъект["РеквизитТекущаяКоманда1_"+НомГруппы]);
	//Данные.Вставить("Команда2",ЭтотОбъект["РеквизитТекущаяКоманда2_"+НомГруппы]);
	Данные.Вставить("Команда1",НаименованиеКоманда1);
	Данные.Вставить("Команда2",НаименованиеКоманда2);
...
Функция ЗаполнениеТабличкиПартий(Данные)
	...
	Возврат Серверные.ТаблицаЗначенийВМассив(ТаблицаПартий);
...
Процедура ТаблицаПартийНачалоВыбораУчастника1(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ЭтотОбъект.ПоказатьВыборИзСписка(Новый ОписаниеОповещения("ТаблицаПартийНачалоВыбораУчастника1Завершение", ЭтотОбъект, Новый Структура("НомГруппы, ТекСтрока", НомГруппы, ТекСтрока)), мПеремСписок,Элемент);
...
Процедура ТаблицаПартийНачалоВыбораУчастника2(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ЭтотОбъект.ПоказатьВыборИзСписка(Новый ОписаниеОповещения("ТаблицаПартийНачалоВыбораУчастника2Завершение", ЭтотОбъект, Новый Структура("НомГруппы, ТекСтрока", НомГруппы, ТекСтрока)), мПеремСписок,Элемент);
...
Процедура ТаблицаПартийОчистка(Элемент,СтандартнаяОбработка)
	...
	Если Элемент.Родитель.ТекущиеДанные["РеквизитКолонкаИтог_"+НомГруппы] = "0" Тогда
...
Процедура ТаблицаПартийОчисткаЗавершение1(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда //удаляем данные из общей таблицы
		...
		ПервыйИгрок = ТекСтрока["РеквизитУчастникИгр_"+НомГруппы];
		ВторойИгрок = ВтораяСтрока["РеквизитУчастникИгр_"+НомГруппы];
		Отбор = Новый Структура;
		...
		Если МассивСтрока.Количество() = 0 Тогда //не нашли пробуем еще
			...
			Если МассивСтрока.Количество() = 0 Тогда
				ПоказатьПредупреждение(Новый ОписаниеОповещения("ТаблицаПартийОчисткаЗавершение", ЭтотОбъект, Новый Структура("ВтораяСтрока, МассивСтрока, НомГруппы, ТекСтрока", ВтораяСтрока, МассивСтрока, НомГруппы, ТекСтрока)), "Не получилось удалить данные");
				Возврат;
			Иначе
		...
		КонецЕсли;
		//ПроставитьДанныеВДеревеТуров(ПервыйИгрок,ВторойИгрок,"","",НомГруппы);
		//обнуляем значения текущей строки и второй строки
		ТаблицаПартийОчисткаФрагмент(ВтораяСтрока, НомГруппы, ТекСтрока);
...
&НаКлиенте
Процедура ТаблицаПартийОчисткаФрагмент(Знач ВтораяСтрока, Знач НомГруппы, Знач ТекСтрока)
	
	Перем Н;
	
	Для Н = 1 По КолПартий Цикл
		ТекСтрока["РеквизитКолонкаИгр_"+Н+"_"+НомГруппы] = 0;
		ВтораяСтрока["РеквизитКолонкаИгр_"+Н+"_"+НомГруппы] = 0;
	КонецЦикла;
	//а теперь итоги
	ТекСтрока["РеквизитКолонкаИтог_"+НомГруппы] = "0";
	ВтораяСтрока["РеквизитКолонкаИтог_"+НомГруппы] = "0";
	ЭтотОбъект.Модифицированность = Истина;
...
Процедура КомандаПоказатьПротокол(Команда)
	...
	Если Искомая.Количество() > 0 Тогда
		...
		ОткрытьФорму("ОбщаяФорма.ФормаВыводаНаПечать",Новый Структура("ТабДок,Заголовок,Ссылка",ФормированиеТаблицГруппКоманд(ПараметрыСтроки),Искомая[0].Комментарий,СсылкаНаСоревнование.Ссылка),ЭтотОбъект,Новый УникальныйИдентификатор,ВариантОткрытияОкна.ОтдельноеОкно);
	КонецЕсли; 
КонецПроцедуры
