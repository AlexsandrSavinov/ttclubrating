...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//проверка общей таблицы 
	//ПроверкаОбщейТаблицыНаНеЗаполненность(); было нужно в переходных версиях
	...
	Если ЗначениеЗаполнено(СсылкаНаДок.ГлавныйСудья) Тогда
		ОбластьПодвал.Параметры.глСудья = Общий.СформироватьИмяИгрока(СсылкаНаДок.ГлавныйСудья);
	КонецЕсли;
	Если ЗначениеЗаполнено(СсылкаНаДок.Секретарь) Тогда
		ОбластьПодвал.Параметры.Секретарь = Общий.СформироватьИмяИгрока(СсылкаНаДок.Секретарь);
	КонецЕсли;
	ТабДок.Вывести(ОбластьПодвал);
	
	ЗаполнитьЗаголовокСпискаИгр();
	//заполняем столы
	//Элементы.ТаблицаСтолов.ВысотаВСтрокахТаблицы = СсылкаНаДок.КолСтолов+1;
	//Для ТекСтол = 1 По СсылкаНаДок.КолСтолов Цикл 
	//	НовСтрока = ТаблицаСтолов.Добавить();
	//	НовСтрока.НомерСтола = ТекСтол;
	//	НовСтрока.КартинкаСтола = БиблиотекаКартинок.СтолПоНастольномуТеннису;
	//КонецЦикла;
...
//проверяет таблицу раньше для сеток не использовались номера групп, теперь, когда перешли на очки
//используются поля так же как и для группового этапа
&НаСервере
Процедура ПроверкаОбщейТаблицыНаНеЗаполненность()
	
	ФлагМод = Ложь;
	Для каждого ТекСтр Из СсылкаНаДок.ОбщаяТаблицаДанных Цикл
		Если ТекСтр.НомерТура = НомерТура И ТекСтр.РежимТура = РежимТура Тогда
			Если ТекСтр.НомерГруппы = 0 И ЗначениеЗаполнено(ТекСтр.НомерСета)Тогда
				ТекСтр.НомерГруппы = ТекСтр.НомерСета;
				ТекСтр.НомерСета = 1;
				ФлагМод = Истина;
			КонецЕсли; 	
		КонецЕсли; 
	КонецЦикла; 
	ЭтотОбъект.Модифицированность = ФлагМод;
...
Процедура ЗаполнениеСоответствийИгроков(Участники)
	...
	Для Каждого ТекИгрок Из Участники Цикл
		...
		НовСтрока.ПредставлениеИгрока = Общий.СформироватьИмяИгрока(ТекИгрок.Участник);
...
&НаСервере
Функция ДобавитьНачальнуюИгру(Данные)
...
&НаКлиенте
Функция УниверсальнаяФункцияПоискаИгрока(Значение)
	
	Если ТипЗнч(Значение) = Тип("Строка") Тогда
		//возможно передается значение с числом очков например: (4:2)
		Скобка = Найти(Значение,"(");
		Если Скобка > 0 Тогда
			Значение = СокрЛП(Сред(Значение,1,Скобка-1));
		КонецЕсли;
		Искомый = СоответствияИгроков.НайтиСтроки(Новый Структура("ПредставлениеИгрока",Значение));
		Если Искомый.Количество() > 0 Тогда
			Возврат Искомый[0].Участник;
		Иначе
			Возврат Неопределено;
		КонецЕсли;
	Иначе
		Искомый = СоответствияИгроков.НайтиСтроки(Новый Структура("Участник",Значение));
		Если Искомый.Количество() > 0 Тогда
			Возврат Искомый[0].ПредставлениеИгрока;
		Иначе
			Возврат Неопределено;
		КонецЕсли;
	КонецЕсли;
...
Процедура ТабДокОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если СсылкаИгрок1 <> Неопределено Или СсылкаИгрок2 <> Неопределено Тогда
		 ...
		 КонецЕсли;
	Иначе
		//ПервыйИгрокТабл = Неопределено;
		//ВторойИгрокТабл = Неопределено;
		//ОчкиПартии1Табл = 0;
		//ОчкиПартии2Табл = 0;
		//ТехническоеВТабл = Ложь;
		//ТехническоеПТабл = Ложь;
...
Процедура ТабДокОбработкаДополнительнойРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если СсылкаИгрок1 <> Неопределено Или СсылкаИгрок2 <> Неопределено Тогда
		...
		КонецЕсли; 
		СтандартнаяОбработка = Ложь;
		ВариантИзСписка = Неопределено;

		ПоказатьВыборИзСписка(Новый ОписаниеОповещения("ТабДокОбработкаДополнительнойРасшифровкиЗавершение", ЭтотОбъект, Новый Структура("СсылкаИгрок1, СсылкаИгрок2", СсылкаИгрок1, СсылкаИгрок2)), ОбновитьКМБыстрогоВвода(СсылкаИгрок1,СсылкаИгрок2),Элементы.ТабДок); 
	КонецЕсли;
...
&НаКлиенте
Процедура БыстрыйВводИзКМ(Команда)
...
Процедура ЗаполнениеТабличкиПартий(Данные)
	...
	//ТехническоеПТабл = Ложь;
    ТаблицаПартий.Очистить();
	ПервыйИгрокТабл = Данные.Игрок1;
	ВторойИгрокТабл = Данные.Игрок2;
	Если Не ЗначениеЗаполнено(ПервыйИгрокТабл) И Не ЗначениеЗаполнено(ВторойИгрокТабл) Тогда
		Возврат;
	КонецЕсли; 
	НовСтрока = ТаблицаПартий.Добавить();
	НовСтрока.Участник = ПервыйИгрокТабл;
	Искомые = 	СсылкаНаДок.СписокУчастников.НайтиСтроки(Новый Структура("Участник",ПервыйИгрокТабл));
	Если Искомые.Количество() > 0 Тогда
		НовСтрока.Рейтинг = Искомые[0].ТекущийРейтинг;
	КонецЕсли; 
	НовСтрока.Итог = 0;
	
	НовСтрока2 = ТаблицаПартий.Добавить();
	НовСтрока2.Участник = ВторойИгрокТабл;
	Искомые2 = 	СсылкаНаДок.СписокУчастников.НайтиСтроки(Новый Структура("Участник",ВторойИгрокТабл));
	Если Искомые2.Количество() > 0 Тогда
		НовСтрока2.Рейтинг = Искомые2[0].ТекущийРейтинг;
	КонецЕсли; 
	НовСтрока2.Итог = 0;
	
	Если ВсяТаблицаОбщихДанных.Количество() = 0 Тогда
		Возврат; //игр нет ничего искать не будем
	КонецЕсли; 
	//теперь ищем в полной таблице все данные Если есть заполняем
	Отбор = Новый Структура;
	Отбор.Вставить("Выигравший",ПервыйИгрокТабл);
	Отбор.Вставить("Проигравший",ВторойИгрокТабл);
	Отбор.Вставить("НомерГруппы",НомерИгрыТабДока);
	МассивОчков = ВсяТаблицаОбщихДанных.НайтиСтроки(Отбор);
	Если МассивОчков.Количество() = 0 Тогда
		//пробуем поменять участников
		Отбор = Новый Структура;
		Отбор.Вставить("Выигравший",ВторойИгрокТабл);
		Отбор.Вставить("Проигравший",ПервыйИгрокТабл);
		Отбор.Вставить("НомерГруппы",НомерИгрыТабДока);
		МассивОчков = ВсяТаблицаОбщихДанных.НайтиСтроки(Отбор);
		Если МассивОчков.Количество() = 0 Тогда
	...
	//ПроставлятьОчки = Истина;
	//Если ПервыйИгрокТабл = ПредопределенноеЗначение("Справочник.Участники.ПустаяСсылка") Тогда
	...
	//		ОчкиПартии2Табл = 3;
	//	КонецЕсли;
	//	ТехническоеВТабл = Истина;
	//	ПроставлятьОчки = Ложь;
	//ИначеЕсли ВторойИгрокТабл = ПредопределенноеЗначение("Справочник.Участники.ПустаяСсылка") Тогда
	//	Если КолПартий = 3 Тогда
	//		ОчкиПартии1Табл = 2;
	//	ИначеЕсли КолПартий = 5 Тогда
	//		ОчкиПартии1Табл = 3;
	...
	//		ОчкиПартии1Табл = 3;
	//	КонецЕсли;
	//	ТехническоеПТабл = Истина;
	//	ПроставлятьОчки = Ложь;
	//КонецЕсли;
	//Если ПроставлятьОчки Тогда
	//	Если Данные.Свойство("Очки1") И Данные.Свойство("Очки2") Тогда //первый игрок выигравший
	//		ОчкиПартии1Табл = Данные.Очки1;
	//		ОчкиПартии2Табл = Данные.Очки2;
	//	КонецЕсли;
	//	Если Данные.Свойство("ТехническоеВ") Тогда
	//		ТехническоеВТабл = Данные.ТехническоеВ;
	//	КонецЕсли;
	//	Если Данные.Свойство("ТехническоеП") Тогда
	//		ТехническоеПТабл = Данные.ТехническоеП;
	//	КонецЕсли;
	//КонецЕсли;
	//ЭтотОбъект.ТекущийЭлемент = Элементы.ОчкиПартии1Табл;
	
	ТаблицаПартий.Сортировать("Рейтинг ВОЗР,Участник ВОЗР");
...
//передается расшифровка вид Игрок1, и возвращает последний номер
&НаКлиенте
Функция ВычислитьНомерИзРасшифровкиСоответствия(Расшифровка)
	
	Если Найти(Расшифровка,"ИграМ") > 0 Тогда
		Возврат СтрЗаменить(Расшифровка,"ИграМ","");
	Иначе  //Игра
		Возврат СтрЗаменить(Расшифровка,"Игра","");
	КонецЕсли;
...
Процедура ЗаполнитьЗаголовокСпискаИгр()
	...
	Если Остаток <> 0 И КолСтолов <> 0 Тогда
		...
		втТекст = Символ(8776)+" "+Общий.ОбработатьСекундыНаВремя(ОбщееВремя);
		втДата = ТекущаяДата();
		//втДатаДляРасчета = СсылкаНаДок.ДатаНачала;
...
&НаКлиенте
Процедура КомандаЗаписатьПартию(Команда)
	
	//Если ОчкиПартии1Табл = ОчкиПартии2Табл Тогда
	//	Возврат;
	//КонецЕсли;
	Строка1 = ТаблицаПартий.Получить(0);
	Строка2 = ТаблицаПартий.Получить(1);
	
	ОчкиПартии1Табл = Строка1.Итог;
	ОчкиПартии2Табл = Строка2.Итог;
	
	Если ОчкиПартии1Табл = ОчкиПартии2Табл Тогда
		Возврат;
	КонецЕсли;
	
	ОдинИЗИтогов = ОпределитьКолПартий();
	Если ОчкиПартии1Табл = ОдинИЗИтогов Или ОчкиПартии2Табл = ОдинИЗИтогов Тогда
	Иначе
		//Предупреждение("Установите правильные очки партий !");
		Возврат;
	КонецЕсли;
	
	Если ОчкиПартии1Табл > ОчкиПартии2Табл Тогда //выиграл первый
		Выигравший  = Строка1.Участник;
		стрВыигравшего = Строка1;
		
		Проигравший = Строка2.Участник;
		стрПроигравшего = Строка2;
		
		ОчкиВыигравшего  = ОчкиПартии1Табл;
		ОчкиПроигравшего = ОчкиПартии2Табл;
	Иначе
		Выигравший  = Строка2.Участник;
		стрВыигравшего = Строка2;
		
		Проигравший = Строка1.Участник;
		стрПроигравшего = Строка1;
		
		ОчкиВыигравшего  = ОчкиПартии2Табл;
		ОчкиПроигравшего = ОчкиПартии1Табл;
	КонецЕсли;
	//заполняем таблицу
	//Отбор = Новый Структура;
	//Отбор.Вставить("Выигравший",Выигравший);
	//Отбор.Вставить("Проигравший",Проигравший);
	//Отбор.Вставить("НомерСета",НомерИгрыТабДока);
	//Отбор.Вставить("НомерТура",НомерТура);
	//МассивСтроки = ВсяТаблицаОбщихДанных.НайтиСтроки(Отбор);
	//Если МассивСтроки.Количество() = 0 Тогда
	//	//пробуем искать подругому
	//	Отбор = Новый Структура;
	//	Отбор.Вставить("Выигравший",Выигравший);
	//	Отбор.Вставить("Проигравший",Проигравший);
	//	Отбор.Вставить("НомерСета",НомерИгрыТабДока);
	//	Отбор.Вставить("НомерТура",НомерТура);
	//	МассивСтроки2 = ВсяТаблицаОбщихДанных.НайтиСтроки(Отбор);
	//	Если МассивСтроки2.Количество() = 0 Тогда
	//		НовСтрока = ВсяТаблицаОбщихДанных.Добавить();
	//	Иначе
	//		НовСтрока = МассивСтроки2[0];
	//	КонецЕсли;
	//Иначе
	//	НовСтрока = МассивСтроки[0];
	//КонецЕсли;
	//НовСтрока.Выигравший  				= Выигравший;
	//НовСтрока.Проигравший 				= Проигравший;
	//НовСтрока.ОчкиВыигравшего  	= ОчкиВыигравшего;
	//НовСтрока.ОчкиПроигравшего 	= ОчкиПроигравшего;
	//НовСтрока.НомерСета	   				= НомерИгрыТабДока;
	//НовСтрока.НомерТура		   			= НомерТура;
	//НовСтрока.ТехническоеВ	        = ТехническоеВТабл;
	//НовСтрока.ТехническоеП			= ТехническоеПТабл;
	//ЭтотОбъект.Модифицированность = Истина;
	//Элементы.ТаблицаОбщихДанных.ТекущаяСтрока = НовСтрока.ПолучитьИдентификатор();
	 //---
	ЭтотОбъект.Модифицированность = Истина;
	ТЧДока = ВсяТаблицаОбщихДанных;
	//попробуем найти эту пару
	...
	ИначеЕсли ИскомыеСтроки.Количество() = 0 Тогда
		//попробуем другой вариант вдруг выигравший изменился
		Отбор = Новый Структура;                               
		Отбор.Вставить("Выигравший",Проигравший);
		Отбор.Вставить("Проигравший",Выигравший);
		Отбор.Вставить("НомерТура",НомерТура);
		Отбор.Вставить("НомерГруппы",НомерИгрыТабДока);
		ИскомыеСтроки = ТЧДока.НайтиСтроки(Отбор);
		Если ИскомыеСтроки.Количество() > 0 Тогда //строки уже есть перезаполняем
	...
	Для Н = 1 По КолПартий Цикл
		...
		Если стрВыигравшего.Техническое Или 	стрПроигравшего.Техническое Тогда //техническое
			...
			НовСтрока.Проигравший = Проигравший;
			НовСтрока.КолШаров	   = 0;
			НовСтрока.ОчкиВыигравшего = ОдинИЗИтогов;
			НовСтрока.ОчкиПроигравшего= 0;
...
&НаКлиенте
Процедура ОчкиПартии1ПриИзменении(Элемент)
	
	ЗначЭлемента = ЭтотОбъект[Элемент.Имя];
	Если ЗначЭлемента > ОпределитьКолПартий() Тогда
		ЭтотОбъект[Элемент.Имя] = 0;
		ЭтотОбъект.ТекущийЭлемент = Элементы[Элемент.Имя];
		ПоказатьОповещениеПользователя("Ошибка",,"Установить правильное значение партий!",БиблиотекаКартинок.Ошибка);
	КонецЕсли;
...
Процедура УдалитьПартию(Команда)
	...
	Строка1 = ТаблицаПартий[0];
	Строка2 = ТаблицаПартий[1];
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("УдалитьПартиюЗавершение", ЭтотОбъект), "Вы уверены, что хотите удалить игру между
	|"+Выигравший+" "+ОчкиВыигравшего+":"+ОчкиПроигравшего+" "+Проигравший+"",РежимДиалогаВопрос.ДаНет);
...
Процедура УдалитьПартиюЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если МассивСтроки.Количество() = 0 Тогда
		...
		Отбор.Вставить("НомерТура",НомерТура);
		МассивСтроки2 = ВсяТаблицаОбщихДанных.НайтиСтроки(Отбор);
		СтрокиДляУдаления = МассивСтроки2;
	Иначе
		СтрокиДляУдаления = МассивСтроки;
	КонецЕсли;
...
Функция ОпределитьКолПартий()
	...
	Возврат Общий.ОпределитьКолПартий(КолПартий);
...
&НаКлиенте
Процедура ТехническоеПриИзменении(Элемент)
	
	ТехническоеВТабл = Ложь;
	ОчкиПартии1Табл =ОпределитьКолПартий();
	ОчкиПартии2Табл = 0;
	
КонецПроцедуры

&НаКлиенте
Процедура Техническое1ПриИзменении(Элемент)//выигравший
	
	ТехническоеПТабл = Ложь;
	ОчкиПартии2Табл = ОпределитьКолПартий();
	ОчкиПартии1Табл = 0;
...
Процедура ВызватьНаСтол(Команда)
	...
	СписокСвободныхСтолов.ПоказатьВыборЭлемента(Новый ОписаниеОповещения("ВызватьНаСтолЗавершение", ЭтотОбъект), "Какой стол?"); 
...
Процедура ТаблицаПартийПередНачаломИзменения(Элемент, Отказ)
	...
	Если Элемент.ТекущийЭлемент.Имя <> "ТаблицаПартийИтог" Тогда
		КороткоеИмя = СтрЗаменить(Элемент.ТекущийЭлемент.Имя,"ТаблицаПартий","");
		глСтароеЗначениеКолонки = ТекСтрока[КороткоеИмя];
		ТекЗначениеСтроки = ТекСтрока[КороткоеИмя];
...
Процедура ТаблицаПартийПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если Элемент.ТекущийЭлемент.Имя = "ТаблицаПартийТехническое" Тогда
		Возврат;
	КонецЕсли; 
	Если Элемент.ТекущийЭлемент.Имя = "ТаблицаПартийИтог" Тогда 
		Если НомТекСтроки = 0 Тогда
			Элементы.ТаблицаПартий.ТекущаяСтрока = ТаблицаПартий.Получить(1).ПолучитьИдентификатор();
		Иначе
			Элементы.ТаблицаПартий.ТекущаяСтрока = ТаблицаПартий.Получить(0).ПолучитьИдентификатор();
		КонецЕсли; 
		Элемент.ТекущийЭлемент = Элементы.ТаблицаПартийИтог;
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ТаблицаПартийТехническоеПриИзменении(Элемент)
	
	ДанныеТаблицы = ТаблицаПартий;
	ТекДанные = Элементы.ТаблицаПартий.ТекущиеДанные;
	ТекСтрока = ДанныеТаблицы.Индекс(ТекДанные);//ТекущиеДанные.ПолучитьИдентификатор();
	ВтораяСтрока = ?(ТекСтрока =0,ДанныеТаблицы.Получить(1),ДанныеТаблицы.Получить(0));
	...
	Если ЗначениеСтроки Тогда
		...
		ВтораяСтрока.Итог = ИтогСтроки;
	КонецЕсли;
	КомандаЗаписатьПартию(Неопределено);
...
Процедура ТаблицаПартийИгра1ПриИзменении(Элемент)
	...
	КонецЕсли;
	СтрокаДляУстановки = ТаблицаШаров.Получить(ИндексМеняемойСтроки);
	КороткоеИмяРеквизита = СтрЗаменить(Элемент.Имя,"ТаблицаПартий","");
	
	ТекЗначение =ТекСтрока[КороткоеИмяРеквизита];
	ЗначДляУстановки = СтрокаДляУстановки[КороткоеИмяРеквизита];
	Если ТекЗначение >=0 Тогда
		Если ТекЗначение >= 10 Тогда
			СтрокаДляУстановки[КороткоеИмяРеквизита] = ТекЗначение+2;
		ИначеЕсли ТекЗначение = 0 Или ТекЗначение = 1 Тогда
			СтрокаДляУстановки[КороткоеИмяРеквизита] = 11;
		ИначеЕсли ТекЗначение >= 2 Или ТекЗначение <= 9 Тогда
			//все хорошо
			СтрокаДляУстановки[КороткоеИмяРеквизита] = 11;
		Иначе
			ПоказатьПредупреждение(Новый ОписаниеОповещения("ТаблицаПартийИгра1ПриИзмененииЗавершение1", ЭтотОбъект, Новый Структура("КороткоеИмяРеквизита, СтрокаДляУстановки, ТекЗначение, ТекСтрока", КороткоеИмяРеквизита, СтрокаДляУстановки, ТекЗначение, ТекСтрока)), "Проверьте правильность введенных очков !");
            Возврат;
		КонецЕсли;
	Иначе //ввели минусовое значение
		...
		Если втТекЗнач >=0 Тогда
			Если втТекЗнач >= 10 Тогда
				ТекСтрока[КороткоеИмяРеквизита] = втТекЗнач+2;
				СтрокаДляУстановки[КороткоеИмяРеквизита] = втТекЗнач;
			ИначеЕсли втТекЗнач = 0 Или втТекЗнач = 1 Тогда
				ТекСтрока[КороткоеИмяРеквизита] = 11;
				СтрокаДляУстановки[КороткоеИмяРеквизита] = втТекЗнач;
			ИначеЕсли втТекЗнач >= 2 Или втТекЗнач <= 9 Тогда
				...
				СтрокаДляУстановки[КороткоеИмяРеквизита] = втТекЗнач;
			Иначе
				ПоказатьПредупреждение(Новый ОписаниеОповещения("ТаблицаПартийИгра1ПриИзмененииЗавершение", ЭтотОбъект, Новый Структура("КороткоеИмяРеквизита, СтрокаДляУстановки, ТекЗначение, ТекСтрока", КороткоеИмяРеквизита, СтрокаДляУстановки, ТекЗначение, ТекСтрока)), "Проверьте правильность введенных очков !");
                Возврат;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	//для сброса значений 
	ТаблицаПартийИгра1ПриИзмененииФрагмент1(КороткоеИмяРеквизита, СтрокаДляУстановки, ТекЗначение, ТекСтрока);
КонецПроцедуры

&НаКлиенте
Процедура ТаблицаПартийИгра1ПриИзмененииЗавершение1(ДополнительныеПараметры) Экспорт
	
	КороткоеИмяРеквизита = ДополнительныеПараметры.КороткоеИмяРеквизита;
	...
	ТекСтрока[КороткоеИмяРеквизита] = глСтароеЗначениеКолонки;
...
&НаКлиенте
Процедура ТаблицаПартийИгра1ПриИзмененииФрагмент1(Знач КороткоеИмяРеквизита, Знач СтрокаДляУстановки, Знач ТекЗначение, Знач ТекСтрока)
	
	ТаблицаПартийИгра1ПриИзмененииФрагмент(КороткоеИмяРеквизита, СтрокаДляУстановки, ТекЗначение, ТекСтрока);

КонецПроцедуры

&НаКлиенте
Процедура ТаблицаПартийИгра1ПриИзмененииЗавершение(ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ТаблицаПартийИгра1ПриИзмененииФрагмент(Знач КороткоеИмяРеквизита, Знач СтрокаДляУстановки, Знач ТекЗначение, Знач ТекСтрока)
	
	Если ТекЗначение = 99 Тогда
		СтрокаДляУстановки[КороткоеИмяРеквизита] = 0;
		ТекСтрока[КороткоеИмяРеквизита] = 0;
	КонецЕсли;
	ПосчитатьИтогиПартийДляТаблицы();
	КомандаЗаписатьПартию(Неопределено);
...
&НаКлиенте
Процедура ТаблицаПартийИтогПриИзменении(Элемент)
...
Процедура ТаблицаМониторингаНомерСтолаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Проверка1_1 = Ложь И Проверка1_2 = Ложь 
		...
		ПоказатьВыборИзСписка(Новый ОписаниеОповещения("ТаблицаМониторингаНомерСтолаНачалоВыбораИзСпискаЗавершение", ЭтотОбъект), СписокСвободныхСтолов,Элемент); 
	КонецЕсли;
КонецПроцедуры
...
Процедура ТаблицаМониторингаНомерСтолаНачалоВыбораИзСпискаЗавершение(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныйСтол <> Неопределено Тогда
		...
		Если ИскомаяИгра.Количество() > 0 Тогда
			...
			ДанныеДляВстреч.Вставить("Участник1",ПервыйИгрокТабл);
			ДанныеДляВстреч.Вставить("Участник2",ВторойИгрокТабл);
...
Процедура КомандаПоказатьПротокол(Команда)
	...
	ОткрытьФорму("ОбщаяФорма.ФормаВыводаНаПечать",Новый Структура("ТабДок,Заголовок,Ссылка",ТабДок,ТекстЗаголовкаТабДока,СсылкаНаДок.Ссылка),ЭтотОбъект,Новый УникальныйИдентификатор,ВариантОткрытияОкна.ОтдельноеОкно);
