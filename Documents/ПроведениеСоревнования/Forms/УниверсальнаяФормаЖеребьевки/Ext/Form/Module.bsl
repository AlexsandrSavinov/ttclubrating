...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтотОбъект.Заголовок = "Жеребьевка "+НомерТура+" тура, "+НРег(РежимТура);
...
Процедура ЗаполнитьИсходныхУчастниковСМестами()
	...
	|	втОбщийСписок.РежимТура,
	|	втСвернутаяТаблица.Участник,
	...
	Для каждого ТекСтр Из Результат Цикл
		Если ТекСтр.РежимТура = Перечисления.РежимыТуровСоревнования.Групповой Тогда
			мОбщаяТаблицаПоТуру = ОбъектДок.ОбщаяТаблицаДанных.Выгрузить(Новый Структура("НомерТура,НомерГруппы",ТекСтр.НомерТура,ТекСтр.НомерГруппы));
			...
			КонецЦикла;
			ТаблицаСМестами = Документы.ПроведениеСоревнования.РасчетМестНаСервере(ДанныеПоМестамУчастников.Скопировать(,"Участник"),мОбщаяТаблицаПоТуру,МестаИзТура);
		...
		Иначе
			...
			КоличествоУчастников = ОбъектДок.УчастникиПоЖеребьевке.НайтиСтроки(Новый Структура("НомерТура,РежимТура",ТекСтр.НомерТура,ТекСтр.РежимТура));
			Ном = 1;
			//заполним таблицу с местами
			Для Мест = 0 По КоличествоУчастников.ВГраница() Цикл
				НовСтрокаДанных = ДанныеПоМестамУчастников.Добавить();
...
&НаСервере
Функция ПолучитьПредыдущийНомерПоРежиму()
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.Текст = "ВЫБРАТЬ * ПОМЕСТИТЬ втТаблица ИЗ &Внеш КАК Внеш";
	Запрос.УстановитьПараметр("Внеш",ОбъектДок.ХодСоревнования.Выгрузить());
	Запрос.Выполнить();
	Текст = "ВЫБРАТЬ
	|	ПроведениеСоревнованияХодСоревнования.НомерСтроки КАК НомерСтроки,
	|	ПроведениеСоревнованияХодСоревнования.РежимТура,
	|	ПроведениеСоревнованияХодСоревнования.ВидСоревнования
	|ИЗ
	|	втТаблица КАК ПроведениеСоревнованияХодСоревнования
	|ГДЕ
	|	ПроведениеСоревнованияХодСоревнования.НомерСтроки < &НомерСтроки
	|	И (ПроведениеСоревнованияХодСоревнования.ВидСоревнования = ЗНАЧЕНИЕ(Перечисление.ВидыСоревнований.Личные)
	|			ИЛИ ПроведениеСоревнованияХодСоревнования.ВидСоревнования = ЗНАЧЕНИЕ(Перечисление.ВидыСоревнований.ПустаяСсылка))
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки УБЫВ";
	Запрос.Текст = Текст;
	Запрос.УстановитьПараметр("НомерСтроки",НомерТура);
	Результат = Запрос.Выполнить().Выгрузить();
...
Процедура ЗаполнитьУФОформлениеОдноклубников(МассивЦветов = Неопределено)
	...
	//Если ВариантРаскраски = 0 Тогда
	//	Элементы.ИсходныеИгрокиУчастникГород.Видимость = Истина;
	//	Элементы.ИсходныеИгрокиУчастникКлуб.Видимость = Ложь;
	//	Элементы.ИсходныеИгрокиУчастникТренер.Видимость = Ложь;
	//	
	//	Элементы.УчастникиВСледующийТурУчастникГород.Видимость = Истина;
	//	Элементы.УчастникиВСледующийТурУчастникКлуб.Видимость	= Ложь;
	//	Элементы.УчастникиВСледующийТурУчастникТренер.Видимость = Ложь;
	//ИначеЕсли ВариантРаскраски = 1 Тогда
	//	Элементы.ИсходныеИгрокиУчастникГород.Видимость = Ложь;
	//	Элементы.ИсходныеИгрокиУчастникКлуб.Видимость = Истина;
	//	Элементы.ИсходныеИгрокиУчастникТренер.Видимость = Ложь;
	//	
	//	Элементы.УчастникиВСледующийТурУчастникГород.Видимость = Ложь;
	//	Элементы.УчастникиВСледующийТурУчастникКлуб.Видимость	= Истина;
	//	Элементы.УчастникиВСледующийТурУчастникТренер.Видимость = Ложь;
	//ИначеЕсли ВариантРаскраски = 2 Тогда   //тренер
	//	Элементы.ИсходныеИгрокиУчастникГород.Видимость = Ложь;
	//	Элементы.ИсходныеИгрокиУчастникКлуб.Видимость = Ложь;
	//	Элементы.ИсходныеИгрокиУчастникТренер.Видимость = Истина;
	//	
	//	Элементы.УчастникиВСледующийТурУчастникГород.Видимость = Ложь;
	//	Элементы.УчастникиВСледующийТурУчастникКлуб.Видимость	= Ложь;
	//	Элементы.УчастникиВСледующийТурУчастникТренер.Видимость = Истина;
	//Иначе
	//	Элементы.ИсходныеИгрокиУчастникГород.Видимость = Ложь;
	//	Элементы.ИсходныеИгрокиУчастникКлуб.Видимость = Ложь;
	//	Элементы.ИсходныеИгрокиУчастникТренер.Видимость = Ложь;
...
&НаСервере
Процедура ЗаполнитьИсходныхИгроковИзСпискаУчастников()
...
&НаСервере
Функция ПровестиЖеребьевкуНаСервереЗмейка(ВведенноеКолГрупп)
...
Процедура ВыполнитьПодготовкуКРежимуСистемаМинус2()
	...
	//Элементы.УчастникиВСледующийТурПропускает1Тур.Видимость = Истина;
	Элементы.УчастникиВСледующийТурНомер.Видимость = Истина;
	Элементы.УчастникиВСледующийТурНомер.КнопкаВыбора = Истина;
	//Если НомерТура = 1 Тогда
	//	ЗаполнитьИсходныхИгроковИзСпискаУчастников();
	//Иначе
	//	ПредыдущийЭтап = ПолучитьПредыдущийНомерПоРежиму();
	//	ЗаполнениеТаблицыДляАвтоматическогоРежима(ОбъектДок.ОбщаяТаблицаДанных.Выгрузить(Новый Структура("НомерТура",ПредыдущийЭтап)));
	//КонецЕсли;
	...
	ИначеЕсли НумерацияСетки = Перечисления.НумерацияВСетке.ПоПорядку Тогда
		...
		Элементы.Декорация5.Видимость = Ложь;
	КонецЕсли;
	Элементы.УчастникиВСледующийТурКомандаУстановитьНомераГрупп.Видимость = Истина;
	Элементы.УчастникиВСледующийТурКомандаУстановитьНомераГрупп.Заголовок = "Установить № участ.";
	ЗаполнитьУФОформлениеОдноклубников();
	
КонецПроцедуры

&НаКлиенте
Функция ОбновитьНадписьИгр()
	
	//Если ЧисловоеЗначениеТура <> 1 Тогда
	//	Если ИтогоУчастниковТура <= 8 Тогда 
	//		Текст = "
	//		|1. 1-8
	//		|
	//		|2. 5-4
	...
	//		|
	//		|4. 7-2";
	//	ИначеЕсли ИтогоУчастниковТура > 8 И ИтогоУчастниковТура <= 16 Тогда
	//		Текст = "
	//		|1. 1-16
	//		|
	//		|2. 9-8
	...
	//		|
	//		|8. 15-2";
	//	ИначеЕсли ИтогоУчастниковТура > 16 И ИтогоУчастниковТура <= 24 Тогда
	//		Текст = "
	//		|1. 1
	//		|
	//		|2. 17-16
	...
	//		|
	//		|16. 2";
	//	ИначеЕсли ИтогоУчастниковТура > 24 И ИтогоУчастниковТура <= 32 Тогда
	//		Текст = "
	//		|1. 1-32
	//		|
	//		|2. 17-16
	...
	//		|
	//		|16. 31-2";
	//	ИначеЕсли ИтогоУчастниковТура > 32 И ИтогоУчастниковТура <= 64 Тогда
	//		Текст = "
	//		|1. 1-64
	//		|2. 33-32
	//		|3. 17-48
	...
	//		|32. 63-2";
	//	КонецЕсли;
	//	Если Элементы.Декорация4.Заголовок <> Текст Тогда
	//		Элементы.Декорация4.Заголовок = Текст;
	//	КонецЕсли;
	//КонецЕсли;
...
&НаСервере
Процедура ЗаполнениеТаблицыДляАвтоматическогоРежима(Данные)
	
	//пока предположим что 1 тур всегда групповой
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.Текст = "ВЫБРАТЬ * ПОМЕСТИТЬ ТЗ ИЗ &Внеш КАК Внеш";
	Запрос.УстановитьПараметр("Внеш",Данные);
	Запрос.Выполнить();

	ТаблицаЗаполнения = Новый ТаблицаЗначений;
	ТаблицаЗаполнения.Колонки.Добавить("Игрок");
	ТаблицаЗаполнения.Колонки.Добавить("Рейтинг");
	ТаблицаЗаполнения.Колонки.Добавить("МестоВГруппе");
	ТаблицаЗаполнения.Колонки.Добавить("НомерГруппы");
	
	Текст = "ВЫБРАТЬ
	|	ПроведениеСоревнованияОбщаяТаблицаДанных.НомерГруппы
	|ИЗ
	|	ТЗ КАК ПроведениеСоревнованияОбщаяТаблицаДанных
	|
	|СГРУППИРОВАТЬ ПО
	|	ПроведениеСоревнованияОбщаяТаблицаДанных.НомерГруппы";
	Запрос.Текст = Текст;
	ВсегоГрупп = Запрос.Выполнить().Выгрузить();
	Для Каждого ТекСтр Из ВсегоГрупп Цикл
		НомГруппы = ТекСтр.НомерГруппы;
		КоличествоУчастников = ОбъектДок.УчастникиПоЖеребьевке.НайтиСтроки(Новый Структура("НомерГруппы,НомерТура,РежимТура",НомГруппы,НомерТура-1,Перечисления.РежимыТуровСоревнования.Групповой));
		...
		Для Мест = 0 По КоличествоУчастников.ВГраница() Цикл
			...
			Для Каждого ТекУч Из КоличествоУчастников Цикл
				...
				ДанныеДляОтбора.Вставить("Первый",КоличествоУчастников[Мест].Участник);
				ДанныеДляОтбора.Вставить("Второй",КоличествоУчастников[ТекНомер-1].Участник);
				ДанныеДляОтбора.Вставить("НомерТура",НомерТура-1);
				РезультатИгры =  Документы.ПроведениеСоревнования.ПоискРезультатИгрыДляГрупповогоЭтапа(ДанныеДляОтбора,ОбъектДок.ОбщаяТаблицаДанных.Выгрузить());
				Если РезультатИгры <> "" Тогда
					ВсегоОчков = ВсегоОчков + Число(РезультатИгры);
				КонецЕсли;
				ТекНомер = ТекНомер + 1;
			КонецЦикла;
			НовСтрокаДанных["Очки"] = ВсегоОчков;
		КонецЦикла;
		Документы.ПроведениеСоревнования.РасчетМестНаСервере(ДанныеПоМестамУчастников,ОбъектДок.ОбщаяТаблицаДанных.Выгрузить(Новый Структура("НомерТура",НомерТура)),НомерТура);
		Для Каждого ТекИгрок Из ДанныеПоМестамУчастников Цикл
			НовСтрока = ТаблицаЗаполнения.Добавить();
			НовСтрока.Игрок 			= ТекИгрок["Участник"];
			НовСтрока.Рейтинг 		= 0;
			НовСтрока.МестоВГруппе  = ТекИгрок["Место"];
			НовСтрока.НомерГруппы   = НомГруппы;
		КонецЦикла;
	КонецЦикла;
	ТаблицаЗаполнения.Сортировать("МестоВГруппе Возр,НомерГруппы Возр,Игрок Возр");
	
	Элементы.ИсходныеИгрокиМестоВГруппе.Видимость = Истина;
	ТЧСписок = ОбъектДок.СписокУчастников.Выгрузить();
	Для Каждого ТекИгрок Из ТаблицаЗаполнения Цикл
		ИскомыйРейтинг = ТЧСписок.Найти(ТекИгрок.Игрок);
		Если ИскомыйРейтинг <> Неопределено Тогда
			ТекИгрок.Рейтинг = ИскомыйРейтинг.ТекущийРейтинг;
		КонецЕсли;
		НовСтрока = ИсходныеИгроки.Добавить();
		НовСтрока.Участник     = ТекИгрок.Игрок;
		НовСтрока.Рейтинг      = ТекИгрок.Рейтинг;
		НовСтрока.МестоВГруппе = ТекИгрок.МестоВГруппе;
	КонецЦикла;
...
Процедура УчастникиВСледующийТурНомерНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	//Если ЧисловоеЗначениеТура = 1 Тогда
		...
		ПоказатьВыборИзСписка(Новый ОписаниеОповещения("УчастникиВСледующийТурНомерНачалоВыбораЗавершение", ЭтотОбъект), СписокКоличестваГрупп,Элемент);
...
&НаКлиенте
Процедура КомандаУстановитьНомераГрупп(Команда)
	...
	Если УчастникиВСледующийТур.Количество() > 0 Тогда
		...
		МассивТипов.Добавить(Тип("ПеречислениеСсылка.НумерацияВСетке"));
		ОписаниеТипов = Новый ОписаниеТипов(МассивТипов);
		ПоказатьВводЗначения(Новый ОписаниеОповещения("КомандаУстановитьНомераГруппЗавершение", ЭтотОбъект, Новый Структура("мНумерация", мНумерация)), мНумерация,"Выберите нумерацию",ОписаниеТипов); 
	 КонецЕсли;
	 
КонецПроцедуры

&НаКлиенте
Процедура КомандаУстановитьНомераГруппЗавершение(Значение, ДополнительныеПараметры) Экспорт
	
	мНумерация = ?(Значение = Неопределено, ДополнительныеПараметры.мНумерация, Значение);
	
	
	Если (Значение <> Неопределено) Тогда
		НумерацияСетки = мНумерация;
...
&НаСервере
Процедура УстановитьАвтоматическиНомераГрупп()
...
Процедура ТабДокПревьюПриАктивизацииОбласти(Элемент)
	...
	Если ТекТекст = "" Тогда
		Возврат;
	КонецЕсли; 
	Фамилия = Общий.ВыделитьСлово(ТекТекст," ");
	Для каждого ТекУчастник Из УчастникиВСледующийТур Цикл
