...
Функция РасчетМестНаСервере(СписокУчастников,ОбщаяТаблицаДокумента,НомерТура = Неопределено) Экспорт
	...
	Для каждого ТекСтр Из СписокУчастников Цикл
		...
		Для Каждого ТекУч Из СписокУчастников Цикл
			...
			ДанныеДляОтбора.Вставить("Первый",СписокУчастников[Индекс].Участник);
			ДанныеДляОтбора.Вставить("Второй",СписокУчастников[ТекНомер-1].Участник);
...
Процедура ЗаполнениеПараметровОбласти(ОбластьТЧ,КолВо)
	
	Частное = КолВо/2;
	Остаток = Частное - Цел(Частное);
	Если Остаток > 0 Тогда
		КолВо = КолВо + 1;//что бы в играх не оставались пустые ячейки
	КонецЕсли;
	
	Для Н = 1 По КолВо Цикл
		Для К = 1 По КолВо Цикл
			Если К = Н Тогда//проверка сам с собой играть не будет
				Продолжить;
			КонецЕсли;
			Попытка
				ИмяПараметра = "И"+Н+"_"+К;
				ОбластьТЧ.Параметры[ИмяПараметра] = ""+Н+" - "+К;
			Исключение //пробуем другое значение
				ИмяПараметра = "И"+К+"_"+Н;
				ОбластьТЧ.Параметры[ИмяПараметра] = ""+К+" - "+Н;
			КонецПопытки;
		КонецЦикла;
	КонецЦикла;
...
Функция ДобавитьНомерМестаИгры(ТаблицаМест,Номер)
	НовСтрока = ТаблицаМест.Добавить();
	НовСтрока.НомерИгры = Номер;
КонецФункции

//ПЕЧАТЬ ТАБЛИЦ ПО ГРУППОВОМУ ЭТАПУ
Функция ФормированиеТаблицГрупп(ТекСтр,СИграми = Ложь,ДокОбъект) Экспорт
	...
	КоличествоГруппПоТуру = ОпределитьКоличествоГрупп(ДокОбъект.УчастникиПоЖеребьевке.Выгрузить(),НомерТура,Перечисления.РежимыТуровСоревнования.Групповой);
	...
	//ОбластьЗаголовокТабДока = ПолучитьОбластьЗаголовка(ТекСтр.Комментарий,ДокОбъект.ДопИнф);
	//ДопИнфа - будет состоять из даты, комментария к туру и до инфы
	//
	...
	Для Н = 1 По КоличествоГруппПоТуру Цикл
		...
		Для К = 0 По КоличествоУчастников.ВГраница() Цикл
			...
			ОбластьСтроки.Параметры.Участник	   = Общий.СформироватьИмяИгрока(КоличествоУчастников[К].Участник);
	...
	Если ЗначениеЗаполнено(ДокОбъект.ГлавныйСудья) Тогда
		Подвал.Параметры.глСудья =  Общий.СформироватьИмяИгрока(ДокОбъект.ГлавныйСудья);
	КонецЕсли;
	Если ЗначениеЗаполнено(ДокОбъект.Секретарь) Тогда
		Подвал.Параметры.Секретарь =  Общий.СформироватьИмяИгрока(ДокОбъект.Секретарь);
	КонецЕсли;
...
Функция ТабличныйДокументРейтинг(ДокСсылка) Экспорт
	...
	Для Каждого ТекСтрока Из ТаблицаРасчета Цикл
		...
		СтрокаТаблицы.Параметры.Участник = Общий.СформироватьИмяИгрока(ТекСтрока.Участник);
...
Функция ТабличныйДокументСписокКоманд(ДокСсылка) Экспорт
	...
	Если ЗначениеЗаполнено(ДокСсылка.ГлавныйСудья) Тогда
		ОбластьПодвал.Параметры.глСудья = Общий.СформироватьИмяИгрока(ДокСсылка.ГлавныйСудья);
	КонецЕсли;
	Если ЗначениеЗаполнено(ДокСсылка.Секретарь) Тогда
		ОбластьПодвал.Параметры.Секретарь = Общий.СформироватьИмяИгрока(ДокСсылка.Секретарь);
	КонецЕсли;
		...
		Для каждого ТекГруппаВывода Из ТаблицаКомандПоТуру Цикл
			...
			Если ОбщиеДанные.Количество() > 0 Тогда
				...
				Для Каждого ТекСтрИгр Из втОбщиеСыгранныеИгрыПоГруппе Цикл //вывод табличек игр
					глВсегоПартийДляПобеды = Общий.ОпределитьКолПартий(ТекТур.КолПартий);
...
Функция ФормированиеТаблицГруппКоманд(ТекСтр,СИграми = Ложь,ДокОбъект) Экспорт
	...
	Для Н = 1 По КоличествоГруппПоТуру Цикл
		...
		Если СИграми Тогда
			...
			//добавим расчет соотношений, по W/L (выигранных/проигранных партий)
			//Для каждого ТекСтрТаблицаДанных Из ДанныеПоМестамУчастников Цикл
				//Если ТекСтрТаблицаДанных["Соотношение"+Н] = "" Тогда
				//	//ищем соотношение партий
				//	перемТекУчастник = ТекСтрТаблицаДанных["РеквизитУчастник"+Н]; 
				//	Если глВыводСоотношенийПартий Тогда
				//		СоотношениеWL =  СформироватьОтношениеВыигранныхКПроиграннымПартиям(перемТекУчастник,мОбщаяТаблицаПоТуру);
				//	Иначе
				//		СоотношениеWL = "";
				//	КонецЕсли; 
				//	ТекСтрТаблицаДанных["Соотношение"+Н] = СоотношениеWL;
				//КонецЕсли; 
			//КонецЦикла; 
		КонецЕсли;
