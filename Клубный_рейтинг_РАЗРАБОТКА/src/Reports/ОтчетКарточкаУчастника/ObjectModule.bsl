...
Функция СформироватьКарточкуУчастника(АдресДанных) Экспорт
	...
	ЗаголовокОбласть.Параметры.ТекРейтинг		=	Строка(СерверныеСервер.ПолучитьТекущийРейтингУчастника(ТекУчастник,ТекущаяДата(),мНастройкиОтчета.ВидРейтинга))+
	?(мНастройкиОтчета.ВидРейтинга = Справочники.ВидыРейтинга.ПустаяСсылка(),""," ("+Строка(НРег(мНастройкиОтчета.ВидРейтинга))+")");
	
	ТекРазряд = СерверныеСервер.ПолучитьРазрядЗваниеУчастника(ТекУчастник,ТекущаяДата());
...
Функция ПолучитьДанныеДляДиаграммы(ТекУчастник,НачПериод,КонПериод,перемВидРейтинга)
	...
	Для Каждого ТекСтрока Из РезультатЗапроса Цикл
		Если ТекСтрока.Рейтинг = Null Тогда
			ТекСтрока.Рейтинг = СерверныеСервер.ПолучитьТекущийРейтингУчастника(ТекСтрока.Участник,ТекСтрока.ДатаДок,ТекСтрока.ВидРейтинга);
		КонецЕсли;
	КонецЦикла;
