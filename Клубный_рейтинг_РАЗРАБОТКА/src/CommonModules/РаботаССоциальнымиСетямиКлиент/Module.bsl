...
Процедура ОткрытьФормуОтправкиВСоцСеть(ИсточникВызова, ТипСоцСети) Экспорт
	...
	ОповещениеПослеЗакрытия = Новый ОписаниеОповещения("ПослеЗакрытияФормыОтправки", РаботаССоциальнымиСетямиКлиент, ПараметрыОтправки);
...
Процедура ОтправитьДокументыВоВконтакте(ДанныеДляОтправки) Экспорт
	...
	Для Каждого ТекЗапись Из ДанныеДляОтправки.ИДДляОтправки Цикл
		...
		СтруктураДанных = Общий.СформироватьОтветHTTPВСтруктуру(Ответ);	//считывание в структуру			
		...
		Если Ответ.КодСостояния = 200 Тогда
			Если СтруктураДанных.Свойство("response") Тогда
				...
				ПараметрыОтвета = Общий.СформироватьОтветHTTPВСтруктуру(ОтветHTTP);
				...
				Если ПараметрыОтвета.Свойство("file") Тогда
					...
					РезультатПубликации = Общий.СформироватьОтветHTTPВСтруктуру(ОтветНаЗапрос);
					...
					Если РезультатПубликации.Свойство("response") Тогда
						...
						РезультатОтправки = Общий.СформироватьОтветHTTPВСтруктуру(ОтветНаРазмещениеЗаписи);
...
//
Функция ОтправитьФайлТелеграммБоту(ДанныеДляОтправки) Экспорт
	...
	Для каждого ТекУчастник Из ДанныеДляОтправки.ИДДляОтправки Цикл
		...
		// Разбор ответа
		Если Ответ.КодСостояния <> 200 Тогда
		    ОтветСтрокой   = Общий.СформироватьОтветHTTPВСтруктуру(Ответ);
...
КонецФункции //ОтправитьФайлТелеграммБоту

Функция ОтправитьФайлТелеграммБотуЧерез_SDK(ДанныеДляОтправки) Экспорт
	...
	Для каждого ТекУчастник Из ДанныеДляОтправки.ИДДляОтправки Цикл
		...
		Если парФормат = "HTML" Тогда
			ПутьКФайлу = ДанныеДляОтправки.ФайлКартинки;
			РезультатОтправки 	= sendPhoto(ТекУчастник.ИД, ДанныеДляОтправки.ФайлКартинки, ДанныеДляОтправки.ТокенБота, парЗаголовок); 
		Иначе
			РезультатОтправки 	= sendDocument(ТекУчастник.ИД, ПутьКФайлу, ДанныеДляОтправки.ТокенБота, парЗаголовок); 
		КонецЕсли;
...
Функция sendDocument(chat_id, ИмяФайлаПолное, AccessToken, ИмяФайла) Экспорт
	...
	Данные.Вставить("ИмяФайла", Серверные.Транслитерация(ТекущийФайл.ИмяБезРасширения)+"."+ТекущийФайл.Расширение);
...
Функция sendPhoto(chat_id, ИмяФайлаПолное, AccessToken, ИмяФайла) Экспорт
	...
	Данные.Вставить("Boundary", "----" + Строка(Новый УникальныйИдентификатор));
	Данные.Вставить("ИмяФайлаПолное", ИмяФайлаПолное);
	Данные.Вставить("ИмяФайла", Серверные.Транслитерация(ТекущийФайл.ИмяБезРасширения)+"."+ВтФайл.Расширение);
