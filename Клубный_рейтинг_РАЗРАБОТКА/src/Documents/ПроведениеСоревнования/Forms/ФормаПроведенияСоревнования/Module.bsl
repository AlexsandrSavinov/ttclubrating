...
Функция ПолучитьКонечныйТабличныйДокумент(ДанныеДляПолучения)
	...
	Для каждого СтрокаРезультата Из РезультатПоНомерамГруппы Цикл
		...
		Для каждого ТекСтрокаУчастника Из мИсходныеСтроки Цикл
			...
			ОбластьСтроки.Параметры.Участник	   = Общий.СформироватьИмяИгрока(ТекСтрокаУчастника.Участник);
	...
	КонецЦикла; 
	 ...
	 ПараметрыВозврата.Вставить("РасчетнаяТаблица", Серверные.ТаблицаЗначенийВМассив(ИтоговаяИсходныеСтроки));
...
Процедура ТабличныйДокументГруппПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	СколькоДолжноБытьПартий = Общий.ОпределитьКолПартий(КолПартий);
	//участники
	...
	Попытка
		...
		Если ЗначениеЗаполнено(ЗначениеРасшф) Тогда
			Если СтрНачинаетсяС(ЗначениеРасшф,"Группа_") Тогда
				...
				//Элементы.УдалитьДанныеИгры.Заголовок = "Удалить игры группы № " + НомерГруппыДляУдаления;
			Иначе
				НомерГруппыДляУдаления = 0;
				//Элементы.УдалитьДанныеИгры.Заголовок = "Удалить игры группы";
			КонецЕсли; 
		КонецЕсли; 
	Исключение
		//ОписаниеОшибки()
