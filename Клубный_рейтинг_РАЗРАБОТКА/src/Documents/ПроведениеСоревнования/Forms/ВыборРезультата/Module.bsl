...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПервыйУчастник 	= ОбщийКлиентСервер.СформироватьИмяИгрока(ТаблицаПартий.Получить(0).Участник);
	ВторойУчастник	= ОбщийКлиентСервер.СформироватьИмяИгрока(ТаблицаПартий.Получить(1).Участник);
...
Процедура ВариантыВыбораКоличестваПартийПриИзменении(Элемент)
	ВариантыВыбораКоличестваПартийПриИзмененииНаСервере();
	ИтогСтроки = ОбщийКлиентСервер.ОпределитьКолПартий(ВариантыВыбораКоличестваПартий);
...
Процедура ТаблицаПартийПриИзменении(Элемент)
	...
	Если ТекЗначение >=0 Тогда
		...
		ИначеЕсли ТекЗначение >= 2 Или ТекЗначение <= 9 Тогда
			...
			СтрокаДляУстановки["Игра" + НомКолонки] = 11;
		Иначе
			ПоказатьПредупреждение(Новый ОписаниеОповещения("ТаблицаПартийПриИзмененииЗавершение", ЭтотОбъект,
				Новый Структура("НомГруппы, НомКолонки, СтрокаДляУстановки, ТекЗначение, ТекСтрока", НомерГруппы,
				НомКолонки, СтрокаДляУстановки, ТекЗначение, ТекСтрока)), "Проверьте правильность введенных очков !");
			Возврат;
		КонецЕсли;
	...
	Если ВыбраннаяСтрока <> Неопределено Тогда
		...
		Если ТипЗнч(ВыбраннаяСтрока.КолПартий1) = Тип("Строка") И ТипЗнч(ВыбраннаяСтрока.КолПартий2) = Тип("Строка") Тогда //это техническое 
			Если ВыбраннаяСтрока.КолПартий1 = "W" И ВыбраннаяСтрока.КолПартий2 = "L" Тогда
				...
				КолПартийВ		= ОбщийКлиентСервер.ОпределитьКолПартий(ВариантыВыбораКоличестваПартий);
				КолПартийП		= 0;
		...
		СтруктураВозврата.Вставить("ТаблицаОчков",СерверныеСервер.ТаблицаЗначенийВМассив(ТаблицаПартий.Выгрузить()));
