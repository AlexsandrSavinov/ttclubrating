...
Процедура КомандаУстановитьНомераГрупп(Команда)
	...
	мНумерация = ПредопределенноеЗначение("Перечисление.НумерацияВСетке.ПоПорядку");
	...
	Если УчастникиВСледующийТур.Количество() > 0 Тогда
		...
		ПоказатьВводЗначения(Новый ОписаниеОповещения("КомандаУстановитьНомераГруппЗавершение", ЭтотОбъект,
			Новый Структура("мНумерация", мНумерация)), мНумерация, "Выберите нумерацию", ОписаниеТипов);
	КонецЕсли;
