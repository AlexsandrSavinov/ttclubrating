...
Процедура ЗаполнениеСоответствийИгроков(Участники)
	...
	Для Каждого ТекИгрок Из Участники Цикл
		...
		НовСтрока.ПредставлениеИгрока = Общий.СформироватьИмяИгрока(ТекИгрок.Участник, ВыводитьПолноеФИО);
...
Процедура ПродвинутьУчастникаДальше(мВсяТаблица,парНомерИгры,УчастникПродвижения,ДвигатьВыигравшего = Истина)
	...
	Пока ПробоватьПродвинуть Цикл
		...
		Иначе
			...
			НовСтрока.ОчкиВыигравшего = Общий.ОпределитьКолПартий(КолПартий);
			НовСтрока.ОчкиПроигравшего= 0;
			НовСтрока.ТехническоеВ = Ложь;
			НовСтрока.ТехническоеП = Истина;
...
Процедура ТабДокОбработкаДополнительнойРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если СсылкаИгрок1 <> Неопределено Или СсылкаИгрок2 <> Неопределено Тогда
		...
		ДляЗаполненияРезультата = Общий.ШаблонСтруктураДляВыбораРезультата();
		...
		Иначе
			...
			КонецЕсли; 
			ПоказатьВопрос(Новый ОписаниеОповещения("ПослеОтветаНаВопросПроПродвижение", ЭтотОбъект, ДляЗаполненияРезультата),ТекстВопроса,РежимДиалогаВопрос.ДаНет,30, КодВозвратаДиалога.Нет,,КодВозвратаДиалога.Нет);
		КонецЕсли; 
	КонецЕсли;
...
Процедура ПослеОтветаНаВопросПроПродвижение(РезультатВопроса, ДопПараметры) Экспорт
	...
	Если РезультатВопроса <> Неопределено Тогда
		Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
			...
			ДанныеДляЗаполнения.Вставить("КолПартийВ",Общий.ОпределитьКолПартий(ДанныеДляЗаполнения.КолПартий));
...
Процедура ОбработкаВыбораЗначенияИзКонтекстногоМеню(Данные)
	...
	Если КолВыигранныхПартий = "W" Тогда
		ВИтог = Общий.ОпределитьКолПартий(КолПартий);
		ПИтог = 0;
		ВТехническое = Ложь;				
		ПТехническое = Истина;
	Иначе
...
Функция ОбновитьКМБыстрогоВвода(парУчастник1,парУчастник2)
	...
	Пробел10 = "          ";
	МаксимумПартий = Общий.ОпределитьКолПартий(КолПартий);
	//1 вариант
		...
		Для мСтол = 1 По КолСтолов Цикл
			...
			Если ИскомаяИграВМониторинге.Количество() > 0 Тогда// игра есть
				...
				Если мПар2 > мПар1 Тогда
					...
					ОбластьСтола.Параметры.Участник1			= Общий.СформироватьИмяИгрока(ИскомаяИграВМониторинге[0].Участник1.Наименование, ВыводитьПолноеФИО);
					ОбластьСтола.Параметры.НомерУч2			= мПар2;
					ОбластьСтола.Параметры.Участник2			= Общий.СформироватьИмяИгрока(ИскомаяИграВМониторинге[0].Участник2.Наименование, ВыводитьПолноеФИО);
				Иначе
					ОбластьСтола.Параметры.НомерУч1			= мПар2;
					ОбластьСтола.Параметры.Участник1			= Общий.СформироватьИмяИгрока(ИскомаяИграВМониторинге[0].Участник2.Наименование, ВыводитьПолноеФИО);
					ОбластьСтола.Параметры.НомерУч2			= мПар1;
					ОбластьСтола.Параметры.Участник2			= Общий.СформироватьИмяИгрока(ИскомаяИграВМониторинге[0].Участник1.Наименование, ВыводитьПолноеФИО);
				КонецЕсли; 
			КонецЕсли; 
