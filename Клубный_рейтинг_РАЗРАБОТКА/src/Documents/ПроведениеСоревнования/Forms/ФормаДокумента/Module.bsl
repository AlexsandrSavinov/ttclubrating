...
Процедура ПерерасчитатьПриростыИИтоговыйРейтинг(ИДСтрокиПерерасчета = Неопределено)
	...
	Если ТекСтрока.КолПартий1 > ТекСтрока.КолПартий2 Тогда
		...
		ТекСтрока.Прирост1 = мКэфВ * мОбъект.РассчитатьПрирост(РейтВыиграв, РейтПроиграв);
		ТекСтрока.Прирост2 = мКэфП * мОбъект.РассчитатьПрирост(РейтВыиграв, РейтПроиграв, Ложь);
	Иначе
		Если Объект.ВидКоэффициента = 0 Тогда
			мКэфВ 	= Объект.Коэфф;
			мКэфП	= Объект.Коэфф;
		...
		ТекСтрока.Прирост2 = мКэфВ * мОбъект.РассчитатьПрирост(РейтВыиграв, РейтПроиграв);
		ТекСтрока.Прирост1 = мКэфП * мОбъект.РассчитатьПрирост(РейтВыиграв, РейтПроиграв, Ложь);
	КонецЕсли;
