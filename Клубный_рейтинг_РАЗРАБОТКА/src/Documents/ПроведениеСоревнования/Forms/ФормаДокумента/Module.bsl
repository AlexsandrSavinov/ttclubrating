	...
	Иначе
		Если ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда // это копирование
			...
			Для каждого ТекСтр Из Объект.СписокУчастников Цикл
				ТекСтр.ТекущийРейтинг = Серверные.ПолучитьТекущийРейтингУчастника(ТекСтр.Участник, ТекущаяДатаСеанса(), Объект.ВидРейтинга);
			КонецЦикла;
...
Процедура ОбновитьРейтингВСпискеУчастников(мДатаОбновления = Неопределено)
	Для Каждого ТекСтр Из Объект.СписокУчастников Цикл
		ТекСтр.ТекущийРейтинг = Серверные.ПолучитьТекущийРейтингУчастника(ТекСтр.Участник, ?(мДатаОбновления
