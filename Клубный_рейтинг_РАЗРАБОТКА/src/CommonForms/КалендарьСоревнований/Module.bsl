...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Если ЗначениеЗаполнено(ПараметрыСеанса.ОписаниеДляМагазина) Тогда
	//	ТекстДляАкцийМагазина = ПараметрыСеанса.ОписаниеДляМагазина;
	//Иначе
	//	Элементы.ТекстДляАкцийМагазина.Видимость = Ложь;
	//КонецЕсли; 
	
КонецПроцедуры
 
&НаСервере
Функция ЗаполнитьСписокДляОформленияСоревнований(Начало,Конец)
...
Процедура ДатаНачалаМесяцаЗапросаВыбор(Элемент, ВыбраннаяДата)
	...
	Если ИскомаяДата.Количество() = 1 Тогда
		ПредставлениеДляСписка = ?(ИскомаяДата[0].Представление = "",Строка(ИскомаяДата[0].Документ),ИскомаяДата[0].Представление);
	...
	ИначеЕсли ИскомаяДата.Количество() > 1 Тогда
		Для Каждого ТекСоревнование Из ИскомаяДата Цикл
			ПредставлениеДляСписка = ?(ТекСоревнование.Представление = "",Строка(ТекСоревнование.Документ),ТекСоревнование.Представление);
			СписокДействий.Добавить(ТекСоревнование.Документ,ПредставлениеДляСписка,,БиблиотекаКартинок.ДокументОбъект);
		КонецЦикла;
		СписокДействий.Добавить("Добавить","Добавить соревнование",,БиблиотекаКартинок.СоздатьЭлементСписка);
	КонецЕсли;
	Если СписокДействий.Количество() = 0 Тогда
		ПараметрыДанных = Новый Структура("ДатаКалендаря",ВыбраннаяДата);
		ОткрытьФорму("Документ.ПроведениеСоревнования.ФормаОбъекта",ПараметрыДанных,,,ВариантОткрытияОкна.ОтдельноеОкно);
	Иначе
		ВыбратьЭлемент = Неопределено;
		СписокДействий.ПоказатьВыборЭлемента(Новый ОписаниеОповещения("ДатаНачалаМесяцаЗапросаВыборЗавершение", ЭтотОбъект, Новый Структура("ВыбраннаяДата", ВыбраннаяДата)), "Выберите действие...");
	КонецЕсли;
