...
Процедура ДействияПриНачалеРаботыСистемыКлиент() Экспорт
	...
	ДанныеПриНачалеРаботы = Серверные.ВыполнитьПроцедурыИФункцииПриНачалеРаботыСистемы();
	...
	КонецЕсли;
	УстановитьЗаголовокКлиентскогоПриложения(ДанныеПриНачалеРаботы.ЗаголовокПриложения);
...
Процедура ОбработатьВызовЗаСтолТекущиеИгры(Данные,УдалениеИгры = Ложь) Экспорт
	...
	Если УдалениеИгры Тогда
		Серверные.УдалитьВстречуВСписке(Данные);
	Иначе
		Серверные.ПоказатьВстречуВСписке(Данные);
	КонецЕсли; 
...
Процедура СохранитьФайл(Формат,ИсточникВызова) Экспорт
	...
	Если ИсточникВызова.ИмяФормы = "ОбщаяФорма.ФормаВыводаНаПечать" Тогда
		...
		ИмяДляСохранения = Серверные.СформироватьИмяДляСохранения(ИсточникВызова.СсылкаНаДокумент);
	Иначе 
...
Процедура СохранитьФайлЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если (ВыбранныеФайлы <> Неопределено) Тогда
		...
		Иначе
			...
			ТабДок.НачатьЗапись(Новый ОписаниеОповещения("ПослеЗаписиФайлаЗавершение",Клиентские,ИмяФайлаСохранения),ИмяФайлаСохранения,ТипФайла);
		КонецЕсли; 
	КонецЕсли;
...
Процедура ПослеЗаписиФайлаЗавершение(Результат,ДопПараметры) Экспорт
	...
	Если Результат Тогда
		ПоказатьОповещениеПользователя("Файл сохранен",Новый ОписаниеОповещения("НажатиеНаОповещение",Клиентские,ДопПараметры),"Файл был сохранен в "+ДопПараметры,БиблиотекаКартинок.Информация);
	КонецЕсли; 
...
Процедура НажатиеНаОповещение(ДопПараметры) Экспорт
	...
	НачатьЗапускПриложения(Новый ОписаниеОповещения("ПослеЗапускаПриложения",Клиентские),ДопПараметры);
...
Процедура СохранитьВКартинку(ИмяВыходногоФайла,ТабДок)
	...
	ТабДок.АвтоМасштаб = Истина;  
	ТабДок.Записать(втФайл,ТипФайлаТабличногоДокумента.HTML);
	
	Кавычка = Символ(34);
	Если Общий.ЭтоWindows() Тогда
		//ИмяВыходногоФайла = "C:\output.png";
		СтрокаКонвертирования = Кавычка+ПараметрыДляСохранения.ПутьКПрограмме + Кавычка + " "+втФайл+" "+Кавычка+ИмяВыходногоФайла+Кавычка;
	ИначеЕсли Общий.ЭтоLinux() Тогда 
		СтрокаКонвертирования = ПараметрыДляСохранения.ПутьКПрограмме + " "+втФайл+" "+Кавычка+ИмяВыходногоФайла+Кавычка;
	КонецЕсли; 
...
Функция СформироватьОтветHTTPВСтруктура(парОтвет) Экспорт
