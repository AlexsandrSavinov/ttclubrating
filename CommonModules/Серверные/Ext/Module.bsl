...
Процедура ПроверкаПользователя(Имя) Экспорт
	...
	Если ЗначениеЗаполнено(Имя) Тогда
		...
		Если ИскомыйПользователь = Справочники.Пользователи.ПустаяСсылка() Тогда
			...
			Сообщение.Сообщить(); 
			ПараметрыСеанса.ТекПользователь = НовЭлемент;
		Иначе
...
Функция ПроверкаВерсииПлатформы() Экспорт
	...
	МинимальнаяПлатформа = "8.3.10.2667";
...
Функция ПолучитьАктивныеИДДляПубликацииВВК() Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	ПубликацияЗаписейВК.ТипПубликации КАК ТипПубликации,
	|	ПубликацияЗаписейВК.ИД КАК ИД
	|ИЗ
	|	РегистрСведений.ПубликацияЗаписейВК КАК ПубликацияЗаписейВК
	|ГДЕ
	|	ПубликацияЗаписейВК.Публиковать
	|	И ПубликацияЗаписейВК.ТипПубликации <> ЗНАЧЕНИЕ(Перечисление.ТипПубликации.ПустаяСсылка)";
	
	Возврат ТаблицаЗначенийВМассив(Запрос.Выполнить().Выгрузить());
	
КонецФункции

Функция ПодготовитьДанныеДляОтправкиВВК(СсылкаНаДокумент = Неопределено) Экспорт
	
	СтруктураОтвета = Новый Структура;
	СтруктураОтвета.Вставить("Токен",ПолучитьТокенВК());
	СтруктураОтвета.Вставить("АктивныеЗаписи",ПолучитьАктивныеИДДляПубликацииВВК());
	Если СсылкаНаДокумент = Неопределено Тогда
		СтруктураОтвета.Вставить("Заголовок","Документ от "+ТекущаяДата());
	Иначе
		Если ТипЗнч(СсылкаНаДокумент) = Тип("ДокументСсылка.ПроведениеСоревнования") Тогда
			Если ЗначениеЗаполнено(СсылкаНаДокумент.НазваниеСоревнования) Тогда
				СтруктураОтвета.Вставить("Заголовок",СсылкаНаДокумент.НазваниеСоревнования);
			Иначе
				СтруктураОтвета.Вставить("Заголовок",Строка(СсылкаНаДокумент));
			КонецЕсли; 
		Иначе
			СтруктураОтвета.Вставить("Заголовок",Строка(СсылкаНаДокумент));
		КонецЕсли; 
